...
from services.llm_service import LLMService, LLMRequest, LLMResponse, LLMProvider, LLMEndpoint
...
class LLMPerformanceTester:
    def __init__(self, use_test_config=False):
        if use_test_config:
            # 使用测试配置
            try:
    ...
    async def _collect_response_async(self, llm, messages, llm_name, sentence_start):
        """异步收集响应数据的辅助方法"""
        chunks = []
        ...
        try:
            ...
            if response.content:
                ...
                # 将响应内容分割成chunks来模拟流式处理
                content = response.content
                chunk_size = max(1, len(content) // 10)  # 分成10个chunk
                for i in range(0, len(content), chunk_size):
                    chunk = content[i:i + chunk_size]
                    chunks.append(chunk)
                    
                    # 检查chunk是否包含错误信息
                    if "异常" in chunk or "错误" in chunk or "502" in chunk.lower():
                        error_msg = chunk.lower()
        ...
        try:
            ...
            try:
                response_chunks, first_token_time = await asyncio.wait_for(
                    self._collect_response_async(llm, messages, llm_name, sentence_start),
                    timeout=10.0
                )
    ...
    async def _test_llm(self, llm_name: str, config: Dict) -> Dict:
        ...
        try:
            ...
            if "api_key" in config and not config["api_key"]:
                print(f"跳过未配置API密钥的 LLM: {llm_name}")
                return {
                    "name": llm_name,
                    "type": "llm",
                    "errors": 1,
    ...
    async def run(self):
        ...
        # LLM 测试任务
        llm_config = self.config.get("llm", {}).get("apis", {})
        if llm_config:
...
async def main():
    import sys
    use_test_config = "--test" in sys.argv
    tester = LLMPerformanceTester(use_test_config=use_test_config)
    await tester.run()
